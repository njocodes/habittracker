...
const CACHE_TTL = 24 * 60 * 60 * 1000; // 24 Stunden (EXTREM!)
const REQUEST_COOLDOWN = 10 * 60 * 1000; // 10 Minuten Cooldown
const POLLING_INTERVAL = 24 * 60 * 60 * 1000; // Nur alle 24 Stunden pollen!

export function useMinimalHabits() {
  ...
  const fetchData = useCallback(async (force = false) => {
    ...
    // EXTREM aggressive cooldown - nur alle 10 Minuten
    if (!force && now - lastRequestTime.current < REQUEST_COOLDOWN) {
      if (cache.current && now - cache.current.timestamp < CACHE_TTL) {
        ...
        setIsLoading(false);
        console.log('📦 Using cached data (within 10min cooldown)');
        return;
      }
    }
    ...
    if (!force && cache.current && now - cache.current.timestamp < CACHE_TTL) {
      ...
      setEntries(cache.current.entries);
      setIsLoading(false);
      console.log('📦 Using cached data (not expired - 24h)');
      return;
    }
    ...
    try {
      ...
      // Verwende ultra-static Route für minimale Requests
      const response = await fetch('/api/habits/ultra-static', { headers });
      ...
      if (response.status === 304) { // Not Modified
        ...
        console.log('📦 Ultra-static data from cache (304 Not Modified)');
      } else if (response.ok) {
        ...
        console.log('🌐 Ultra-static data fetched and cached for 24 HOURS');
      } else {
        const errorData = await response.json();
        setError(errorData.error || 'Failed to fetch ultra-static data');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      console.error('❌ Error fetching ultra-static data:', err);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    // EXTREM seltenes Polling - nur alle 24 Stunden!
    pollingInterval.current = setInterval(() => {
      console.log('🔄 24h polling triggered');
      fetchData();
    }, POLLING_INTERVAL);
  ...
  const addHabit = async (habitData: Omit<Habit, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Cache für 24 Stunden invalidieren
        cache.current = null;
